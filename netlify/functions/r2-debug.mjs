// r2-debug.mjs — diagnóstico temporário do R2 (REMOVA após resolver!)
...
function getAmzDate(date)   { return date.toISOString().replace(/[-:]/g, "").replace(/\.\d{3}Z$/, "Z").slice(0, 16); }
...
export const handler = async (event) => {
    ...
    try {
      ...
      const result = await new Promise((resolve, reject) => {
        const req = https.request({
          hostname: endpoint, path: `/${R2_BUCKET}/${key}`, method: "PUT", timeout: 7000,
          headers: {
            "Content-Type": "text/plain", "Content-Length": testBuffer.length,
            "x-amz-date": amzDate, "x-amz-content-sha256": bodyHash, "Authorization": authorization,
          },
        }, (res) => {
          const chunks = [];
  ...
  return {
    statusCode: 200,
    headers,
    body: JSON.stringify({ vars, endpoint, uploadTest, amzDateSample: getAmzDate(new Date()) }, null, 2),
  };
};
